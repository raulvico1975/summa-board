...
test('normalizeBlogContentHtml strips a duplicated lead h1 and upgrades markdown strong markers', () => {
  const html = `
    <h1>Per què les remeses són el punt més fràgil de la gestió d'una associació</h1>
    <p>El primer és saber **a qui girar el rebut cada mes**.</p>
  `
  ...
  assert.equal(
    result,
    '<p>El primer és saber <strong>a qui girar el rebut cada mes</strong>.</p>'
  )
})
...
test('normalizeBlogContentHtml does not alter markdown markers inside code blocks', () => {
  const html = '<pre><code>**no tocar**</code></pre><p>Però **això sí**.</p>'

  const result = normalizeBlogContentHtml(html, 'Títol')

  assert.equal(result, '<pre><code>**no tocar**</code></pre><p>Però <strong>això sí</strong>.</p>')
})
