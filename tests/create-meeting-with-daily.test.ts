...
test("poll page links meetings only when a usable meeting exists", async () => {
  ...
  assert.equal(source.includes("const canDeletePoll ="), true);
  assert.equal(source.includes("poll.status === \"closed\" && !!winningOption && isBeforeTodayInAppTimeZone(winningOption.startsAt)"), true);
  assert.equal(source.includes("<DeleteMeetingButton pollId={poll.id} redirectHref={dashboardHref} />"), true);
});
